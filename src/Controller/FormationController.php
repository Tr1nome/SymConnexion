...
class FormationController extends AbstractController
{
    ...
    /**
     * @Route("/{id}/export", name="formation_export", methods= "GET")
     */
    public function exportAction(Formation $formation)
    {
        $em= $this->getDoctrine()->getManager();
        $users = $formation->getUser();
        #Writer
        $writer = new Writer();
        $csv = $writer::createFromFileObject(new \SplTempFileObject());
        $csv->insertOne(['Nom','adresse mail','adhérent ?']);
        foreach ($users as $user) {
            
            $csv->insertOne([$user->getUsername(), $user->getEmail(), $user->isEnabled()]);
        }
